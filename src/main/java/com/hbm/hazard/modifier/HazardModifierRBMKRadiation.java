...
public class HazardModifierRBMKRadiation extends HazardModifier {
	...
	public float modify(ItemStack stack, EntityLivingBase holder, float level) {
		...
		if(stack.getItem() instanceof ItemRBMKRod) {
			...
			double depletion = this.linear ? 1D - ItemRBMKRod.getEnrichment(stack) : 1D - Math.pow(ItemRBMKRod.getEnrichment(stack), 2);
			...
			double depletion = this.linear ? (ItemRBMKPellet.rectify(stack.getItemDamage()) % 5) / 4F : 1 - Math.pow((4 - ItemRBMKPellet.rectify(stack.getItemDamage()) % 5) / 4F, 2);
			
			level = level + (this.target - level) * ((ItemRBMKPellet.rectify(stack.getItemDamage()) % 5) / 4F);
