...
public class Mark5Ausf2 implements IExplosionLogic {
	...
	private void initRays() {
		...
		double bow = this.length * Math.PI;								//one "bow", the distance between the poles along the circumference ( U/2 )
		...
		for(int i = 0; i < bowCount; i++) {
			...
			double ringRadius = Math.sin(currBow) * this.length;										//the radius of the current ring
	...
	private void repopulate() {
		...
		double bow = this.length * Math.PI;
		...
		for(int i = 0; i < bowCount; i++) {
			...
			double ringRadius = Math.sin(currBow) * this.length;
			...
			if(parentSegment != null) {
				...
				for(int j = 0; j < ringCount; j++) {
					...
					if(parentRay != null && parentRay.power > 0) {
						...
						Vec3 len = Vec3.createVectorHelper(parentRay.x - this.originX, parentRay.y - this.originY, parentRay.z - this.originZ);
