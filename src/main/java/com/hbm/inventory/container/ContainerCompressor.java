...
public class ContainerCompressor extends Container {
	...
	public ContainerCompressor(InventoryPlayer playerInv, TileEntityMachineCompressor tile) {
		this.compressor = tile;
		
		//Fluid ID
		addSlotToContainer(new Slot(tile, 0, 17, 72));
		//Battery
		addSlotToContainer(new Slot(tile, 1, 152, 72));
		//Upgrades
		addSlotToContainer(new Slot(tile, 2, 52, 72));
		addSlotToContainer(new Slot(tile, 3, 70, 72));
		...
		if(var4 != null && var4.getHasStack()) {
			...
			if(index < 4) {
				...
				if(var3.getItem() instanceof IBatteryItem) {
					...
					if(!mergeItemStack(var5, 2, 4, false)) {
						return null;
					}
				}
			}
