...
public class ContainerRadiolysis extends Container {
	...
	public ContainerRadiolysis(InventoryPlayer playerInv, TileEntityMachineRadiolysis tile) {
		this.radiolysis = tile;
		
		//RTG
		for(byte i = 0; i < 2; i++) {
			for(byte j = 0; j < 5; j++) {
				addSlotToContainer(new Slot(tile, j + i * 5, 188 + i * 18, 8 + j * 18));
			}
		}
		
		//Fluid IO
		addSlotToContainer(new Slot(tile, 10, 34, 17));
		addSlotToContainer(new SlotTakeOnly(tile, 11, 34, 53));
		
		//Sterilization
		addSlotToContainer(new Slot(tile, 12, 148, 17));
		addSlotToContainer(new SlotTakeOnly(tile, 13, 148, 53));
		
		//Battery
		addSlotToContainer(new Slot(tile, 14, 8, 53));
	...
	public boolean canInteractWith(EntityPlayer player) {
		return this.radiolysis.isUseableByPlayer(player);
	}
