...
public class ContainerMixer extends Container {
	...
	public ContainerMixer(InventoryPlayer player, TileEntityMachineMixer mixer) {
		...
		//Battery
		addSlotToContainer(new Slot(mixer, 0, 23, 77));
		//Item Input
		addSlotToContainer(new Slot(mixer, 1, 43, 77));
		//Fluid ID
		addSlotToContainer(new Slot(mixer, 2, 117, 77));
		//Upgrades
		addSlotToContainer(new Slot(mixer, 3, 137, 24));
		addSlotToContainer(new Slot(mixer, 4, 137, 42));

		for(int i = 0; i < 3; i++) {
			for(int j = 0; j < 9; j++) {
				addSlotToContainer(new Slot(player, j + i * 9 + 9, 8 + j * 18, 122 + i * 18));
			}
		}

		for(int i = 0; i < 9; i++) {
			addSlotToContainer(new Slot(player, i, 8 + i * 18, 180));
		}
	}
