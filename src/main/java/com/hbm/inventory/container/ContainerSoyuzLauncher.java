...
public class ContainerSoyuzLauncher extends Container {
	...
	public ContainerSoyuzLauncher(InventoryPlayer invPlayer, TileEntitySoyuzLauncher tedf) {
		...
		//Soyuz
		addSlotToContainer(new Slot(tedf, 0, 62, 18));
		//Designator
		addSlotToContainer(new Slot(tedf, 1, 62, 36));
		//Satellite
		addSlotToContainer(new Slot(tedf, 2, 116, 18));
		//Landing module
		addSlotToContainer(new Slot(tedf, 3, 116, 36));
		//Kerosene IN
		addSlotToContainer(new Slot(tedf, 4, 8, 90));
		//Kerosene OUT
		addSlotToContainer(new Slot(tedf, 5, 8, 108));
		//Peroxide IN
		addSlotToContainer(new Slot(tedf, 6, 26, 90));
		//Peroxide OUT
		addSlotToContainer(new Slot(tedf, 7, 26, 108));
		//Battery
		addSlotToContainer(new Slot(tedf, 8, 44, 108));
		
		for(int i = 0; i < 3; i++)
		{
			for(int j = 0; j < 6; j++)
			{
				addSlotToContainer(new Slot(tedf, j + i * 6 + 9, 62 + j * 18, 72 + i * 18));
			}
		}
