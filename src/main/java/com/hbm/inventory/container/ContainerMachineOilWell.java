...
public class ContainerMachineOilWell extends Container {
	...
	public ContainerMachineOilWell(InventoryPlayer invPlayer, TileEntityOilDrillBase tedf) {
		...
		// Canister Output
		addSlotToContainer(new SlotTakeOnly(tedf, 2, 80, 53));
		// Gas Input
		addSlotToContainer(new Slot(tedf, 3, 125, 17));
		// Gas Output
		addSlotToContainer(new SlotTakeOnly(tedf, 4, 125, 53));
		//Upgrades
		addSlotToContainer(new Slot(tedf, 5, 152, 17));
		addSlotToContainer(new Slot(tedf, 6, 152, 35));
		addSlotToContainer(new Slot(tedf, 7, 152, 53));
		...
		if(var4 != null && var4.getHasStack()) {
			...
			if(par2 <= 7) {
				...
				if(var5.getItem() instanceof ItemMachineUpgrade) {
					if(!mergeItemStack(var5, 5, 8, true)) {
						return null;
					}
				} else {
	...
	public boolean canInteractWith(EntityPlayer player) {
		return this.well.isUseableByPlayer(player);
	}
}
