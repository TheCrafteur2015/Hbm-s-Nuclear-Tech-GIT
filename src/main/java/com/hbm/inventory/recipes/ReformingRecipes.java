...
public class ReformingRecipes extends SerializableRecipe {
	...
	public void registerDefaults() {
		...
		ReformingRecipes.recipes.put(Fluids.LIGHTOIL, new Triplet(
				new FluidStack(Fluids.AROMATICS, 50),
				new FluidStack(Fluids.REFORMGAS, 10),
				new FluidStack(Fluids.HYDROGEN, 15)
				));
		ReformingRecipes.recipes.put(Fluids.LIGHTOIL_CRACK, new Triplet(
				new FluidStack(Fluids.AROMATICS, 50),
	...
	public void writeRecipe(Object recipe, JsonWriter writer) throws IOException {
		...
		writer.name("output1"); SerializableRecipe.writeFluidStack(rec.getValue().getX(), writer);
		writer.name("output2"); SerializableRecipe.writeFluidStack(rec.getValue().getY(), writer);
		writer.name("output3"); SerializableRecipe.writeFluidStack(rec.getValue().getZ(), writer);
	}
