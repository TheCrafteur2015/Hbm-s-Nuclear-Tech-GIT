...
public class GUILeadBox extends GuiContainer {
	...
	public void drawScreen(int x, int y, float interp) {
		...
		} else if(this.mc.thePlayer.getHeldItem() != this.firstHeld) {
	...
	protected void drawGuiContainerForegroundLayer(int i, int j) {
		...
		if(this.inventory.hasCustomInventoryName()) {
			name = this.inventory.box.getDisplayName();
		}
	...
	protected void drawGuiContainerBackgroundLayer(float p_146976_1_, int p_146976_2_, int p_146976_3_) {
		...
		Minecraft.getMinecraft().getTextureManager().bindTexture(GUILeadBox.texture);
