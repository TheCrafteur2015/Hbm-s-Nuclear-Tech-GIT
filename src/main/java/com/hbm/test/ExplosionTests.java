...
public class ExplosionTests {
	...
	public static void runTest() {
		...
		for(int radius : radii) {
			...
			MK5Frame noBuf = new MK5Frame(ExplosionTests.world, x, y, z, strength, length).setBuffer(new BufferNone());
			...
			TimeAnalyzer.dump();
			System.out.println("Mem diff: " + ((ExplosionTests.getMem() - mem) / 1_048_576) + "MB");
			
			for(double threshold : thresholds) {
				ExplosionTests.BUFFER_THRESHOLD = threshold;

				System.gc();
				mem = ExplosionTests.getMem();
				System.out.println("#### STARTING TEST WITH MAP-BASED PROXIMITY BUFFER " + radius + " / " + threshold + " ####");
				MK5Frame mapBuf = new MK5Frame(ExplosionTests.world, x, y, z, strength, length).setBuffer(new BufferMap());
				...
				MK5Frame arrayBuf = new MK5Frame(ExplosionTests.world, x, y, z, strength, length).setBuffer(new BufferArray(x, y, z, (int) (length)));
