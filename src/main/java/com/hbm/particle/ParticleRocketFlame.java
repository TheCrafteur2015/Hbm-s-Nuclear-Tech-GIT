...
public class ParticleRocketFlame extends EntityFX {
	...
	public ParticleRocketFlame(TextureManager p_i1213_1_, World p_i1218_1_, double p_i1218_2_, double p_i1218_4_, double p_i1218_6_) {
		...
		this.maxAge = 300 + this.rand.nextInt(50);
	}
	...
	public void renderParticle(Tessellator p_70539_1_, float p_70539_2_, float p_70539_3_, float p_70539_4_, float p_70539_5_, float p_70539_6_, float p_70539_7_) {
        ...
        for(int i = 0; i < 10; i++) {
            ...
            float dark = 1 - Math.min(((float)(this.age) / (float)(this.maxAge * 0.25F)), 1);
           
            this.particleRed = 1 * dark + add;
            this.particleGreen = 0.6F * dark + add;
            this.particleBlue = 0 + add;
           
            this.particleAlpha = (float) Math.pow(1 - Math.min(((float)(this.age) / (float)(this.maxAge)), 1), 0.5);
            ...
            float spread = (float) Math.pow(((float)(this.age) / (float)this.maxAge) * 4F, 1.5) + 1F;
           
            float scale = urandom.nextFloat() * 0.5F + 0.1F + ((float)(this.age) / (float)this.maxAge) * 2F;
            float pX = (float) ((this.prevPosX + (this.posX - this.prevPosX) * (double)p_70539_2_ - EntityFX.interpPosX) + (urandom.nextGaussian() - 1D) * 0.2F * spread);
            float pY = (float) ((this.prevPosY + (this.posY - this.prevPosY) * (double)p_70539_2_ - EntityFX.interpPosY) + (urandom.nextGaussian() - 1D) * 0.5F * spread);
            float pZ = (float) ((this.prevPosZ + (this.posZ - this.prevPosZ) * (double)p_70539_2_ - EntityFX.interpPosZ) + (urandom.nextGaussian() - 1D) * 0.2F * spread);
