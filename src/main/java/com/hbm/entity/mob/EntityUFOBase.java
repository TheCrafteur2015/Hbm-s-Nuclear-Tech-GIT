...
public abstract class EntityUFOBase extends EntityFlying implements IMob {
	...
	protected void entityInit() {
		super.entityInit();
		//XYZ
		getDataWatcher().addObject(17, 0);
		getDataWatcher().addObject(18, 0);
		getDataWatcher().addObject(19, 0);
	}
	...
	protected void scanForTarget() {
		...
		if(this.scanCooldown <= 0) {
			List<Entity> entities = this.worldObj.getEntitiesWithinAABB(Entity.class, this.boundingBox.expand(range, range / 2, range));
			this.target = null;
	...
	protected void setCourseForTaget() {
		...
		double overshoot = 10 + this.rand.nextDouble() * 10;
		...
		setWaypoint(wX, Math.max(this.worldObj.getHeightValue(wX, wZ), (int) this.target.posY) + targetHeightOffset(),  wZ);
	}
	...
	protected void setCourseWithoutTaget() {
		int x = (int) Math.floor(this.posX + this.rand.nextGaussian() * 5);
		int z = (int) Math.floor(this.posZ + this.rand.nextGaussian() * 5);
		setWaypoint(x, this.worldObj.getHeightValue(x, z) + wanderHeightOffset(),  z);
	}
