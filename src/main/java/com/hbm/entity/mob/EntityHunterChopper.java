...
public class EntityHunterChopper extends EntityFlying implements IMob, IBossDisplayData, IRadiationImmune {
	...
	public boolean attackEntityFrom(DamageSource source, float amount) {
		...
		if(isEntityInvulnerable() || source instanceof EntityDamageSource || getHealth() <= 0.1F) {
			return false;
		} else if(amount >= getHealth()) {
			initDeath();
			setIsDying(true);
			setHealth(0.1F);
			return false;
		...
		if(this.rand.nextInt(15) == 0) {
			if(!this.worldObj.isRemote && !this.isDying) {
		...
		for(int j = 0; j < 3; j++) {
			double d0 = this.rand.nextDouble() / 20 * this.rand.nextInt(2) == 0 ? -1 : 1;
			double d1 = this.rand.nextDouble() / 20 * this.rand.nextInt(2) == 0 ? -1 : 1;
			double d2 = this.rand.nextDouble() / 20 * this.rand.nextInt(2) == 0 ? -1 : 1;

			for(int i = 0; i < 8; i++)
				if(this.worldObj.isRemote)
					this.worldObj.spawnParticle("fireworksSpark", this.posX, this.posY, this.posZ, d0 * i * 0.25, d1 * i * 0.25, d2 * i * 0.25);
		}
	...
	protected void updateEntityActionState() {
		...
		if(!this.isDying) {
			...
			if(d3 < 1.0D || d3 > 3600.0D) {
				if(this.targetedEntity != null) {
					...
					this.waypointY = this.worldObj.getHeightValue((int) this.waypointX, (int) this.waypointZ) + 10 + this.rand.nextInt(15);
				} else {
			...
			if(this.targetedEntity == null || this.attackCounter <= 0) {
				...
				// 100.0D);
				this.targetedEntity = Library.getClosestEntityForChopper(this.worldObj, this.posX, this.posY, this.posZ, 250);
			}
			...
			if(this.targetedEntity != null && this.targetedEntity.getDistanceSqToEntity(this) < d4 * d4) {
				...
				if(this.attackCounter % 2 == 0 && this.attackCounter >= 120) {
					this.worldObj.playSoundAtEntity(this, "hbm:weapon.osiprShoot", 10.0F, 1.0F);
					...
					Vec3 vec2 = Vec3.createVectorHelper(d5 - 1 + this.rand.nextInt(3), d6 - 1 + this.rand.nextInt(3), d7 - 1 + this.rand.nextInt(3)).normalize();
					double motion = 3;
					...
					entityarrow.setDamage(3 + this.rand.nextInt(5));
				...
				if(this.attackCounter == 80) {
					this.worldObj.playSoundAtEntity(this, "hbm:entity.chopperCharge", 5.0F, 1.0F);
				}

				this.mineDropCounter++;
				if(this.mineDropCounter > 100 && this.rand.nextInt(15) == 0) {
					this.worldObj.playSoundAtEntity(this, "hbm:entity.chopperDrop", 15.0F, 1.0F);
					EntityChopperMine mine = new EntityChopperMine(this.worldObj, this.posX, this.posY - 0.5, this.posZ, 0, -0.3, 0, this);
					this.mineDropCounter = 0;
					this.worldObj.spawnEntityInWorld(mine);

					if(this.rand.nextInt(3) == 0) {
						EntityChopperMine mine1 = new EntityChopperMine(this.worldObj, this.posX, this.posY - 0.5, this.posZ, 1, -0.3, 0, this);
						EntityChopperMine mine2 = new EntityChopperMine(this.worldObj, this.posX, this.posY - 0.5, this.posZ, 0, -0.3, 1, this);
						EntityChopperMine mine3 = new EntityChopperMine(this.worldObj, this.posX, this.posY - 0.5, this.posZ, -1, -0.3, 0, this);
						EntityChopperMine mine4 = new EntityChopperMine(this.worldObj, this.posX, this.posY - 0.5, this.posZ, 0, -0.3, -1, this);
			...
			if(!this.worldObj.isRemote) {
				...
				PacketDispatcher.wrapper.sendToAllAround(new AuxParticlePacketNT(data, this.posX, this.posY, this.posZ),  new TargetPoint(this.dimension, this.posX, this.posY, this.posZ, 100));
			}
		...
		if(this.targetedEntity == null) {
			...
			float f3 = MathHelper.sqrt_double(this.motionX * this.motionX + this.motionZ * this.motionZ);
			if(this.rotationYaw - (float) (Math.atan2(this.posX - this.targetedEntity.posX, this.posZ - this.targetedEntity.posZ) * 180.0D / Math.PI) >= 10)
				this.prevRotationYaw = this.rotationYaw -= 10;
			if(this.rotationYaw - (float) (Math.atan2(this.posX - this.targetedEntity.posX, this.posZ - this.targetedEntity.posZ) * 180.0D / Math.PI) <= -10)
				this.prevRotationYaw = this.rotationYaw += 10;
			if(this.rotationYaw - (float) (Math.atan2(this.posX - this.targetedEntity.posX, this.posZ - this.targetedEntity.posZ) * 180.0D / Math.PI) < 10 && this.rotationYaw - (float) (Math.atan2(this.posX - this.targetedEntity.posX, this.posZ - this.targetedEntity.posZ) * 180.0D / Math.PI) > 10)
				this.prevRotationYaw = this.rotationYaw = (float) (Math.atan2(this.posX - this.targetedEntity.posX, this.posZ - this.targetedEntity.posZ) * 180.0D / Math.PI);
		...
		if(this.rotationPitch <= 330 && this.rotationPitch >= 30) {
			if(this.rotationPitch < 180)
				this.rotationPitch = 30;
			if(this.rotationPitch >= 180)
				this.rotationPitch = 330;
		}
	}
	...
	public void initDeath() {
		...
		if(!this.isDying)
			this.worldObj.playSoundAtEntity(this, "hbm:entity.chopperDamage", 10.0F, 1.0F);
		this.isDying = true;
	}
